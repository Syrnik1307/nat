...
# Инцидент: медленный старт урока (Zoom) — 24 января 2026

## Симптом
- Старт урока занимал ~30 секунд, открывалась пустая вкладка

## Причина
- Python `requests` выбирал IPv6, который таймаутился
- Из-за этого запрос к Zoom API стабильно зависал на ~30 секунд

## Решение
- Принудительно использовать IPv4 в [teaching_panel/schedule/zoom_client.py](teaching_panel/schedule/zoom_client.py)

## Результат
- Время создания встречи: ~30s ➜ ~2-3s

## Примечание (чтобы не повторилось)
- Если появится задержка ~10-30s при запросах к Zoom, первым делом проверять IPv6/IPv4
