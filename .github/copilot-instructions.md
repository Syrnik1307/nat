...
- **Database**: SQLite (dev) / PostgreSQL (production) / Azure Cosmos DB (optional, feature-flagged)
...
1. **Бэкап**: `ssh tp 'sudo -u postgres pg_dump -Fc teaching_panel -f /tmp/backup_$(date +%Y%m%d_%H%M%S).pgdump'`
2. **Проверка**: `ssh tp 'pg_restore --list /tmp/backup_*.pgdump | wc -l'` — должно быть > 10 объектов
...
**Деплой скрипт**: `.\deploy_to_production.ps1` автоматически создаёт бэкап `/tmp/deploy_*.pgdump` и требует ввести "MIGRATE" для подтверждения миграций.

**Восстановление из бэкапа:**
```bash
ssh tp 'sudo -u postgres pg_restore --clean --dbname=teaching_panel /tmp/deploy_YYYYMMDD_HHMMSS.pgdump && sudo systemctl restart teaching_panel'
```
...
# Database
DATABASE_URL=postgresql://user:pass@host:5432/dbname  # PostgreSQL on production & staging
...
- **db-guardian** — безопасность миграций, бэкапы PostgreSQL
