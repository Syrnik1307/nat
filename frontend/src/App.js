...
const OlgaAuthPage = lazy(() => import('./components/olga/OlgaAuthPage'));
const OlgaDashboard = lazy(() => import('./components/olga/OlgaDashboard'));
const OlgaCourseCatalog = lazy(() => import('./components/olga/OlgaCourseCatalog'));
const OlgaCourseView = lazy(() => import('./components/olga/OlgaCourseView'));
const OlgaCourseAdmin = lazy(() => import('./components/olga/OlgaCourseAdmin'));
const OlgaCourseManager = lazy(() => import('./components/olga/OlgaCourseManager'));
const OlgaProfile = lazy(() => import('./components/olga/OlgaProfile'));
const OlgaNavBar = lazy(() => import('./components/olga/OlgaNavBar'));
...
const AppRoutes = () => {
  ...
  const hideNavPaths = ['/auth-new', '/register', '/verify-email', '/olga/auth'];
  ...
  const shouldShowStudentNav = baseNavVisible && isStudentView && !isOlgaDomain;
  // В админке используем собственный хедер страницы, верхний общий navbar не показываем
  const shouldShowNav = baseNavVisible && !isStudentView && !isAdminHomeView && !isOlgaDomain;
  const shouldShowOlgaNav = isOlgaDomain && !["/olga/auth"].includes(location.pathname);
  ...
  // === Olga Domain Guard ===
  // Redirect all non-olga paths to olga equivalents when on Olga domain
  if (isOlgaDomain && !location.pathname.startsWith('/olga/')) {
    const allowedOnOlga = ['/register', '/verify-email', '/reset-password', '/simple-reset'];
    const isAllowedPath = allowedOnOlga.some(p => location.pathname.startsWith(p));
    if (!isAllowedPath) {
      const olgaTarget = !accessTokenValid ? '/olga/auth' :
        (role === 'teacher' || role === 'admin') ? '/olga/admin' : '/olga/courses';
      return <Navigate to={olgaTarget} replace />;
    }
  }
  // === End Olga Domain Guard ===

  const RootRedirect = () => {
    const { accessTokenValid, role, loading } = useAuth();
    if (loading) return <AuthCheckingSkeleton />;
    if (!accessTokenValid) return <Navigate to={isOlgaDomain ? '/olga/auth' : '/auth-new'} replace />;
    if (isOlgaDomain) {
      if (role === 'teacher' || role === 'admin') return <Navigate to='/olga/admin' replace />;
      if (role === 'student') return <Navigate to='/olga/courses' replace />;
      return <Navigate to='/olga/auth' replace />;
    }
    ...
    <>
      ...
      <Suspense fallback={<PageLoader />}>
        <Routes>
          ...
          {/* Olga domain routes */}
          <Route path="/olga/courses" element={<OlgaDashboard />} />
          <Route path="/olga/catalog" element={<OlgaCourseCatalog />} />
          <Route path="/olga/course/:id" element={<OlgaCourseView />} />
          <Route path="/olga/admin" element={<OlgaCourseAdmin />} />
          <Route path="/olga/admin/courses" element={<OlgaCourseManager />} />
          <Route path="/olga/profile" element={<OlgaProfile />} />
