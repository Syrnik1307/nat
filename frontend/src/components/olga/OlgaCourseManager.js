            ...
            <div key={course.id} className="ocm-card" onClick={() => openEditor(course)}>
              <div className="ocm-card-cover">
                {course.cover_url ? (
                  ...
                  <div className="ocm-card-placeholder">Фото</div>
                )}
              ...
              <div className="ocm-card-body">
                ...
                <div className="ocm-card-meta">
                  ...
                  {course.price && <span>Цена: {course.price} ₽</span>}
                </div>
              </div>
              <button
    ...
    <div className="ocm-editor">
      ...
      <div className="ocm-tabs">
        ...
        <button className={`ocm-tab ${activeTab === 'constructor' ? 'active' : ''}`}
          onClick={() => setActiveTab('constructor')}>Содержание</button>
      </div>
            ...
            <div className="ocm-cover-section">
              <div className="ocm-cover-preview">
                {course?.cover_url ? (
                  ...
                  <span className="ocm-cover-placeholder-icon">Фото</span>
                )}
              </div>
              <div>
  ...
  <div className="ocm-lesson-row">
    ...
    <div className="ocm-lesson-info">
      ...
      <div className="ocm-lesson-badges">
        ...
        {lesson.is_free_preview && <span className="ocm-badge preview">Превью</span>}
        {lesson.materials && lesson.materials.length > 0 && (
          <span className="ocm-badge file">Файлы: {lesson.materials.length}</span>
        )}
      </div>
    </div>
    <div className="ocm-lesson-actions">
      ...
      <button className="ocm-icon-btn" onClick={onEdit} title="Редактировать">Ред.</button>
      <button className="ocm-icon-btn danger" onClick={onDelete} title="Удалить">Удал.</button>
    </div>
  </div>
);
              ...
              <div className="ocm-materials-list">
                {materials.map(mat => (
                  <div key={mat.id} className="ocm-material-row">
                    <a href={mat.url} target="_blank" rel="noopener noreferrer">Файл: {mat.name}</a>
                    <button className="ocm-icon-btn danger" onClick={() => handleDeleteMaterial(mat.id)}>Удал.</button>
                  </div>
                ))}
              </div>
            )}
